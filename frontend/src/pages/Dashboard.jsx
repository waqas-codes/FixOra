...
const Dashboard = () => {
  ...
  const stats = [
    ...
    { label: 'Completed Requests', value: requests.filter(r => r.status === 'Completed').length },
  ];
                ...
                <tbody className="">
                  {requests.map((req) => (
                    <tr key={req._id || req.id} className="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm text-slate-600 dark:text-slate-400">{req.location}</td>
                      <td className="px-6 py-4">
